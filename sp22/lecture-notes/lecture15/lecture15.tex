...
%Custom Title Fields
\newcommand{\lectTitle}{Lecture 15 Notes}
...
\subsection*{Tricks}
    ...
    \vspace{1.5mm} 
    Now $10^{2022} = 10^{6 \cdot 337} \equiv 1 \pmod{7}$, since the sequence repeats every $6$ values.
    
\subsection*{Fermat's Little Theorem}
    To introduce this theorem, we first state a lemma.
    
    \vspace{1.5mm}
    \textbf{Lemma.} Take some prime number $p$ and some integer $a$ not divisible by $p$. Then $\{i \cdot a\}_{1 \le i \le p - 1}$ are all distinct $\pmod{p}$.
    
    \textit{Proof.} (By contradiction) Assume there are $1 \le i, j \le p - 1$ such that $ia \equiv ja \pmod{p}$, so $(i - j)a \equiv 0 \pmod{p}$, which is a contradiction! (unless $i = j$).
    ...
    \vspace{1.5mm}
    \textbf{Fermat's Little Theorem.} For any prime number $p$ and integer $a$ where $p \nmid a$, $a^{p - 1} \equiv 1 \pmod{p}$.
...
\subsection*{Chinese Remainder Theorem}
    ...
    There exists exactly one $x$ (taken mod $M$, where $M = m_1 m_2 \dots m_k$) such that 
    \begin{align*}
    ...
    \textit{Proof. } We construct a solution using $M = m_1 m_2 \dots m_k$ and $$M_{i} = \frac{M}{m_{i}}.$$ First set $y_{i} \in \mathbb{Z}$ such that $y_{i} \cdot M_{i} \equiv 1 \pmod{m_{i}}$.
