...
\newcommand{\mhl}[1]{{\setlength{\fboxsep}{0pt}\colorbox{lightgray}{$#1$}}}
...
\section*{Cardinality}
    ...
    \begin{enumerate}[label = \arabic*.]
        \item Reflexivity: There exists a bijection from $A$ to $A$, $f: A \rightarrow A$.
        \item Symmetry: Since $A\mathcal{R}B$, we have defined a bijection from $A$ to $B$. As discussed before, bijections have inverses, so we can define an inverse $f^{-1}: B \rightarrow A$, so $B \mathcal{R} A$.
    ...
    \vspace{1.5mm}
    \textbf{Proof.}
    We know $A \times B = \{(a_{i}, b_{j}) : a_{i} \in A, b_{j} \in B\}$.
    
    Define a function $f$ by ordering the pairs along diagonals where the sum of the indices $i+j$ is constant:
    \begin{align*}
        &f(a_{1}, b_{1}) = 1, \\
        &f(a_{2}, b_{1}) = 2, \quad f(a_{1}, b_{2}) = 3, \\
        &f(a_{3}, b_{1}) = 4, \quad f(a_{2}, b_{2}) = 5, \quad f(a_{1}, b_{3}) = 6, \\
        &f(a_{4}, b_{1}) = 7, \quad f(a_{3}, b_{2}) = 8, \quad f(a_{2}, b_{3}) = 9, \quad f(a_{1}, b_{4}) = 10,
    \end{align*}
    and so on. Since every pair $(a_i, b_j)$ belongs to exactly one diagonal and each diagonal is finite, every pair is assigned a unique natural number. Thus, $f: A \times B \rightarrow \mathbb{N}$ is a bijection.
