...
\subsection*{Solution}
    ...
    \textsc{Inductive Step}: Now let $n \in \mathbb{N}$ such that $P(n)$ is true. We wish to prove $P(n + 1)$ is true as well; in other words, we wish to show $$1 + 2 + \dots + n + (n + 1) = \frac{(n + 1)(n + 2)}{2}.$$ We focus on the LHS. Note that by the inductive hypothesis, $$(1 + 2 + \dots + n) + (n + 1) = \frac{n(n + 1)}{2} + (n + 1) = \frac{n(n + 1)}{2} + \frac{2(n + 1)}{2} = \frac{(n + 2)(n + 1)}{2}.$$
...
\subsection*{Solution}
    ...
    Let $S = 1 + r + r^{2} + \dots + r^{n}$. Multiplying both sides by $r$ gives:
    $$rS = r + r^{2} + r^{3} + \dots + r^{n + 1}.$$
    Subtracting $rS$ from $S$ yields:
    $$S - rS = 1 - r^{n + 1}.$$
    Factoring out $S$ and dividing by $1 - r$ (assuming $r \ne 1$), we get:
    $$S = \frac{1 - r^{n + 1}}{1 - r}.$$
...
\subsection*{Solution}
    ...
    \textsc{Inductive Step}: Now let $n \ge 4$ such that $P(n)$ is true. We wish to prove $P(n + 1)$ is true as well; in other words, we wish to show $$2^{n+1} \le (n+1)!$$ We focus on the LHS. Note that by the inductive hypothesis, $$2^{n+1} = 2 \cdot 2^n \le 2 \cdot n! < (n+1) \cdot n! = (n+1)!$$
    where we used that $2 < n + 1$ for all $n \ge 4$. Hence $P(n + 1)$ is true as well. \\
    \textsc{Conclusion}: Therefore by induction, for all $n \ge 4$, $P(n)$ is true.


\subsection*{Example 4 (TBC)}
    Let $n \in \mathbb{W}$. Define the harmonic numbers $H_{n}$, as $$H_{n} = \frac{1}{1} + \frac{1}{2} + \frac{1}{3} + \dots + \frac{1}{n}.$$ Use induction to prove $$H_{2^{n}} \ge 1 + \frac{n}{2}.$$
...
\subsection*{Solution}
    ...
    $$\begin{aligned}[t]
        ...
        &\ge \left(1 + \frac{n}{2}\right) + 2^{n} \cdot \frac{1}{2^{n + 1}} \quad \text{(since there are $2^{n}$ terms, each $\ge \frac{1}{2^{n + 1}}$)} \\ 
...
\subsection*{Solution}
Solution covered in Lecture 9 Notes
...
\subsection*{Solution}
    ...
    \textsc{Base Case}: $P(0)$ is true, since $$\int_0^\infty x^0e^{-x}dx = \lim_{a \to \infty} \int_0^a e^{-x}dx = \lim_{a \to \infty}\left[ -e^{-x} \right]_0^a = (0) - (-1) = 1 = 0!.$$
    \textsc{Inductive Step}: Now let $n \in \mathbb{W}$ such that $P(n)$ is true. Then using integration by parts, we have $$\int_0^\infty x^{n + 1}e^{-x}dx = \lim_{a \to \infty} \left[ -x^{n + 1}e^{-x}\right]_0^a + (n + 1)\int_0^\infty x^ne^{-x}dx = 0 + (n + 1)(n!) = (n + 1)!$$ Hence $P(n + 1)$ is true as well. \\
    \textsc{Conclusion}: Therefore by induction, for all $n \in \mathbb{W}$, $P(n)$ is true.
