...
\subsection*{Calculator Example}
    ...
    \begin{algorithmic}
        \If{$n = 1$}
            \State return $a$
        \ElsIf{$n > 1$}
            \State return Calculator$(a, n - 1) \times a$
        \EndIf
    \end{algorithmic}
    \end{algorithm}
    ...
    \begin{algorithmic}
        \If{$n = 1$}
            \State return $a$
        \ElsIf{$n$ is even}
            \State return $Cal(a, \tfrac{n}{2}) \times Cal(a, \tfrac{n}{2})$
        \ElsIf{$n$ is odd}
            \State return $Cal(a, \tfrac{n - 1}{2}) \times Cal(a, \tfrac{n - 1}{2}) \times a$
        \EndIf
    \end{algorithmic}
    \end{algorithm}
    
    The recurrence formula for this algorithm is $T(n) \le T(n/2) + 2 \text{ (2 is worst case)}$. Consider $n = 2^{k}$. Then $$T(n) = T(\tfrac{n}{2}) + 1, T(1) = 0.$$
    ...
    \textsc{Base Case}: $P(0)$ is true, since $T(1) = 0 = \log_{2}(1)$ \\
    \textsc{Inductive Step}: Now let $k \in \mathbb{N}$ such that $P(k)$ is true. Then $T(2^{k + 1}) = T(\tfrac{2^{k + 1}}{2}) + 1 = T(2^{k}) + 1 = \log_{2}(2^{k}) + 1 = k + 1 = \log_{2}(2^{k+1})$. So $P(k + 1)$ is true as well. \\
    \textsc{Conclusion}: Therefore by induction, for all $k \ge 0$, $P(k)$ is true.
