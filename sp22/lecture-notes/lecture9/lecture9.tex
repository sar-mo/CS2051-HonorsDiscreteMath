...
\subsection*{Solution}
    ...
    \textsc{Inductive Step}: Now let $k \in \{2\ell + 1 : \ell \ge 1\}$ such that $P(k)$ is true. We wish to show $P(k + 2)$ is true. Observe that as in the base case, there will always be two players who have the smallest distance between them, and thus will always pass to each other. Call these players the Pair.
    \begin{Cases}
...
\section*{Strong Induction}
    The Principle of Strong Induction is a variant of induction where, in the inductive step, we assume that the statement holds for \textit{all} values up to $k$, rather than just $k$. 
    
    It works by showing $(\forall k)(P(1) \land P(2) \land \dots \land P(k) \Rightarrow P(k + 1))$. Because we assume the truth of all previous cases, it provides a stronger foundation to prove $P(k + 1)$ while remaining logically equivalent to standard induction.
...
\subsection*{Example 7 Solution (Continued)}
    ...
    \textsc{Inductive Step}: Now let $k \ge 21$ such that $P(18), P(19), \dots, P(k)$ are true. To make change for $k + 1$, simply add a $4$ Cuban bill to the change made for $k - 3$. \\
...
\subsection*{Solution}
    ...
    \textsc{Inductive Step}: Now let $n \in A$ such that $P(2), P(3), \dots, P(n)$ are all true. We wish to prove that $P(n + 1)$ is also true.
    ...
    \textsc{Conclusion}: We have proved by strong induction that for each $n \in A$, $P(n)$ is true.
...
\subsection*{Question 2}
    The \textit{Well-Ordering Principle} states that every non-empty subset of the natural numbers $\mathbb{N}$ has a least element. Show that this principle and the principle of induction are equivalent using a two-sided proof.
...
\subsection*{Solution}
    ...
    \textsc{Base Case}: $P(1)$ is true. This is because $S$ is a subset of $\mathbb{N}$, and since $1$ is the least element of $\mathbb{N}$, if $1 \in S$, then $1$ would be the least element of $S$. But $S$ does not have a least element. Therefore $1 \notin S$. \\
    \textsc{Inductive Step}: Now let $n \in \mathbb{N}$ such that $P(1), P(2), \dots, P(n)$ are all true. This means $1 \notin S, 2 \notin S, \dots, n \notin S$. Note that $n + 1$ is the next smallest integer of $\mathbb{N}$ after $1, 2, \dots, n$. Therefore $(n + 1) \notin S$, because otherwise it would be the least element of $S$. Hence $P(n + 1)$ is also true. \\
