...
Let's motivate this section by recalling some explication from a prior supplement:
    ...
    \begin{center}
        \begin{tikzpicture}
          ...
          \draw[thick,fill=white] (H)++(50:0.6) circle (0.06*\H);
          \draw[thick,fill=white] (H)++(40:1) circle (0.08*\H);
          \draw[thick,fill=white] (H)++(45:1.5) circle (0.1*\H);

          \node[cloud, draw =black, fill = gray!10, aspect=3.5, cloud puffs = 15, cloud puff arc = 90, text width=9cm] (c) at (8,1.5) {\small But, how can you evaluate arbitrary expressions without even knowing the names of the variables ahead of time? This question itself could be an entire supplemental, but luckily Python makes the job easy with the \lstinline+eval()+ function.
