...
\section*{Overview}
    ...
    \end{tcolorbox}
    Initially, I was rather confused. Noam Chomsky may be many things, including a world-renowned linguist and psychologist, but it was difficult to see his correlation to discrete mathematics. However, as I looked more into the topic, I realized the true importance of his discoveries, and how they have revolutionized not only the field of linguistics but also computer science as a whole.
...
\begin{tcolorbox}[enhanced,interior style={top color=Dandelion!20,bottom color=Dandelion!30}]
    Let's build our own \lstinline{eval()} function. \textbf{Implement the following functions:}
    \begin{itemize}
...
\subsection*{Context-Free Grammars}
    ...
    \end{enumerate}
    The language of the grammar is then $L_{\textsf{CFG}} = \{w \in \Sigma^{*} : S \rightarrow u_{1} \rightarrow u_{2} \rightarrow \dots \rightarrow w\}$; put another way, it is the set of all valid strings generated by repeated application of our rules containing no variables.
...
\begin{tcolorbox}[enhanced,interior style={top color=Dandelion!20,bottom color=Dandelion!30}]
    Let's practice generating CFGs. \textbf{Implement the following functions:}
...
\section*{Part 3: Syntatical Structures (10 points)}
    ...
    \vspace{2mm}
    Thus English can also be (somewhat) modeled as a context-free grammar. In this section, we provide a brief overview of how this modeling works.
    ...
    \vspace{2mm}
    Personally, I think one of the clearest examples of CFGs in English arises when combining sentences using conjunctions, as in the following: 
    ...
    \vspace{2mm}
    While these simple examples provide a cursory introduction, further resources should be consulted to gain a deeper understanding of the complex syntactical structures needed for the English language (and to complete the implementation requirements). I believe perusing \href{http://www.cs.columbia.edu/~mcollins/cs4705-spring2020/slides/parsing1.pdf}{this} set of slides or reading \href{http://faculty.washington.edu/cicero/370syntax.htm}{this} introduction should be sufficient.
...
\begin{tcolorbox}[enhanced,interior style={top color=Dandelion!20,bottom color=Dandelion!30}]
    ...
    \begin{itemize}
        \item \lstinline{generate_cfg_logic(atoms)} In Part 1, we created a parser for propositions by recursively splitting them based on the logical operators. We can create a CFG for this set of strings in a similar fashion. The alphabet of your grammar should be over $\text{\lstinline{atoms}} \cup \{\land, \lor, \neg, \Rightarrow, \Leftrightarrow, (, )\}$. Here \lstinline{atoms} is a set of lowercase letters representing the propositions over which our language is defined. You may insert more parentheses than necessary. You must generate all syntactically valid regular expressions.
        \item
