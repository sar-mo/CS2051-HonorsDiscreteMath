...
% \section*{Sample Answer Format}
    ...
    % \vspace{2mm}
    % \textsc{Inductive Step:} Now let $n \in \mathbb{N}$ such that $P(n)$\footnote{For strong induction, this would be $P(\cdot), P(\cdot + 1), \dots, P(n)$} is true. \dots. Therefore, $P(n + 1)$ is true as well.

    % \vspace{2mm}
    % \textsc{Conclusion:} Therefore by [strong] induction, for all $n \in \{\mathbb{N}, \mathbb{Z}, \text{etc}\}$, $P(n)$ is true.
