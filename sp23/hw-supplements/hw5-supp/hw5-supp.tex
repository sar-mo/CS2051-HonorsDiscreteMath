...
\tikzset{  
    -Latex,auto,node distance =1.3 cm and 1.1 cm, thick,% node distance is the distance between one node to other, where 1.5cm is the length of the edge between the nodes  
...
\section*{Part 2: Partitioning with Equivalence Relations (10 pts)}
    ...
    \subsection*{Example: Vector Spaces}
        ...
        \begin{figure}[H]
            ...
            \caption{The two matrices above have the same row rank. Thus under row equivalence, they are partitioned into the same equivalence class.}
            \label{fig:1}
        \end{figure}
        ...
        \begin{figure}[H]
            ...
            \caption{The canonical forms of each of the equivalence classes corresponding to matrix equivalence taken over the set of $2 \times 2$ matrices.}
            \label{fig:2}
        \end{figure}
        ...
        \begin{figure}[H]
            ...
            \begin{subfigure}{0.45\textwidth}
                ...
                \caption{$H$ and $\hat{H}$ are matrix equivalent.}
            \end{subfigure}
            \begin{subfigure}{0.45\textwidth}
                \centering
                \includegraphics[scale = .4]{sp23/hw-supplements/hw5-supp/images/linalg_matrix_similarity_equiv_classes.png}
                \caption{The partitions of matrix similarity.}
            \end{subfigure}
...
\section*{Part 3: Single-Processor Task Scheduling (10 pts)}
    ...
    \left\{
    \begin{array}{ccc}
        (1301, 1301), & (1301, 1331), & (1301, 1332)\\
        (1331, 1331), & (1331, 1332), & (1301, 4641) \\
        (1332, 1332), & (2051, 3511), & (1301, 3600) \\
        (2051, 2051), & (1332, 3511), & (1301, 3511) \\
        (4641, 4641), & (1332, 3600), & (1331, 4641) \\
        (3511, 3511), & (1332, 4641), & (1331, 3600) \\
        (3511, 3600), & (1301, 1301) & (1331, 3511) \\
    \end{array}
    \right\}$$
    However, this doesn't really provide us with much intuition. Since this relation is a partial order, we can visualize it using something called a \textbf{dependency graph}.
    ...
    \vspace{3mm}
    \textbf{Definition:} A \textit{topological sorting} is a total ordering of a partially ordered set.
...
\section*{Part 4: Multi-Processor Task Scheduling (10 pts)}
    ...
    % \vspace{2mm}
    % The nice thing about posets is that this is always possible! In other words, for any poset, there is a legal parallel schedule that runs in $t$ steps, where $t$ is the length of the longest chain. This is known as Mirsky's theorem, and the proof can be found in the textbook.
    ...
    If we wanted to do this in general, one way would be to run \lstinline{generate_schedule} for all \lstinline{num_processors} from $1$ to \lstinline{len(elements)}, and then return the minimum, like so:
    \begin{lstlisting}
