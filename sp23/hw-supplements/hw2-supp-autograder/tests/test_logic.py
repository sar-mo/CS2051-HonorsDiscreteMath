# test_logic.py
# CS 2051 Spring 2023 - HW2 Supplement : Python Primer
# Instructor: Gerandy Brito
# author: Sarthak Mohanty

from gradescope_utils.autograder_utils.decorators import weight, number

import logic_solutions as ls
from logic import *
...
class TestLogic(unittest.TestCase):
    ...
    def test_count_satisfying(self):
        ...
        for prop in self.props:
            self.assertEqual(count_satisfying(prop), ls.count_satisfying(prop))
    ...
    def test_are_equivalent(self):
        ...
        for prop1, prop2 in pairs:
            self.assertEqual(are_equivalent(prop1, prop2), ls.are_equivalent(prop1, prop2))

    @weight(3)
    @number("1.3")
    def test_is_tautology(self):
        for prop in self.props:
            self.assertEqual(is_tautology(prop), ls.is_tautology(prop))
        
    @weight(3)
    @number("1.4")
    def test_is_contradiction(self):
        for prop in self.props:
            self.assertEqual(is_contradiction(prop), ls.is_contradiction(prop))
    
    @weight(3)
    @number("1.5")
    def test_is_contingency(self):
        for prop in self.props:
            self.assertEqual(is_contingency(prop), ls.is_contingency(prop))
    ...
    def test_model_fitting(self):
        ...
        for i in range(2, len(self.props)):
            prop = self.props[i]
            self.assertEqual(truth_table(model_fitting(truth_table(prop))), truth_table(prop), msg=f"Model fitting failed on hidden test case")