        ...
        with mock.patch('sys.stdout', new = StringIO()) as std_out:
            for i in range(len(self.primes)):
                ...
                self.assertEqual(petty_thief.brute_force(sol_alice.computePublicKey()),\
                                    sol_petty_thief.brute_force(sol_alice.computePublicKey()), msg= 'BadActor class not working')

                # test baby-step giant-step
                self.assertEqual(crime_boss.bsgs(sol_alice.computePublicKey()),\
                                    sol_crime_boss.bsgs(sol_alice.computePublicKey()), msg= 'BadActor class not working')
                ...
                self.assertEqual(petty_thief._secret_key, secret, msg= 'BadActor class not working')
