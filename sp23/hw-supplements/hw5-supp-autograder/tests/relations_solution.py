...
# Some of the docstrings are outside of the method. This is intentional, to ensure all methods are completed in one line.
...
def partition(elements: set, equiv_relation: bool) -> list[set]: 
    ...
    for e in elements: # Loop over each element
        ...
        for p in partitions:
            if equiv_relation(e, next(iter(p))): # Found a partition for it!
                p.add(e)
                found = True
                break
    ...
    # do it in one line using lambda functions
